        ...
        <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
                                    ...
                                    <table class="table table-striped table-hover" id="vaccineTable">
                                        ...
                                        <tbody>
                                            <c:forEach var="listTypesOfVaccine" items="${listTypesOfVaccine}">
                                                <tr>
                                                    <td>${listTypesOfVaccine.typeID}</td>
                                                    <td>${listTypesOfVaccine.name}</td>
                                                    <td>
                                                        <a href="UpdateTypeOfVaccineServlet?typeID=${listTypesOfVaccine.typeID}"
                                                           class="settings" title="Sửa" data-toggle="tooltip"><i class="fas fa-edit" style="margin:0 7px; color:#299BE4;"></i></a>
                                                        <a href="CRUDTypeOfVaccineServlet?action=delete&typeID=${listTypesOfVaccine.typeID}" 
                                                           onclick="confirmDelete(event, this)" class="delete" title="Xóa" data-toggle="tooltip">
            ...
            <script>
                    ...
                    $(document).ready(function () {
                        var rowsPerPage = 10;
                        var rows = $('#vaccineTable tbody tr');
                        var rowsCount = rows.length;
                        var pageCount = Math.ceil(rowsCount / rowsPerPage);
                        var numbers = $('#pagination');

                        for (var i = 1; i <= pageCount; i++) {
                            numbers.append('<a href="#" class="page-number">' + i + '</a>');
                        }
