 ...
 * @author Xuan Vinh
 */
@WebServlet(name = "EditVaccineInWarehouseServlet", urlPatterns = {"/EditVaccineInWarehouseServlet"})
public class EditVaccineInWarehouseServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditVaccineInWarehouseServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditVaccineInWarehouseServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (warehouseIDStr != null && !warehouseIDStr.isEmpty()) {
            ...
            VaccineWarehouse warehouse = vaccineWarehouseDAO.getWarehouseById(warehouseID);
            if (warehouse != null) {
                request.setAttribute("warehouse", warehouse);
                request.setAttribute("vaccines", vaccines);
                request.setAttribute("workLocations", workLocations);
                request.getRequestDispatcher("editVaccineInWarehouse.jsp").forward(request, response);
            } else {
                response.sendRedirect("ManagementVaccineInWarehouseServlet");
            }
        } else {
            response.sendRedirect("ManagementVaccineInWarehouseServlet");
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean success = vaccineWarehouseDAO.updateWarehouse(warehouseID, vaccineID, workLocationID, quantity);
        if (success) {
            ...
            request.setAttribute("errorMessage", "Chỉnh sủa số lượng vaccine trong kho thất bại!");
            request.getRequestDispatcher("editVaccineInWarehouse.jsp").forward(request, response);
        }
    }
